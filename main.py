...
def get_teams_possession(df: pd.DataFrame, teams: list[str], max_frame: int = None):

    possession = {
        teams[0]: 0,
        teams[1]: 0
    }   
    # dict of this structure: {frame_nbr: assigned_id}
    possession_analyze_dict = df.groupby(['frame_nbr']).agg(['max'])['assigned_id'].to_dict()['max']

    frame_counter = 0
    for frame, assigned_id in possession_analyze_dict.items():
        ...
        if max_frame is not None and frame > max_frame:
            break
        frame_counter += 1    
        current_team = df.loc[df.assigned_id == assigned_id].iloc[0]['player_team']
        possession[current_team] += 1

    possession['max_frames'] = frame_counter

    possession[teams[0]] = int((possession[teams[0]] / possession['max_frames']) * 100)
    possession[teams[1]] = int((possession[teams[1]] / possession['max_frames']) * 100)
...
def ball_assigner(bboxes_p, labels_p, p_ids, ball_bbox_xywh):
    ...
    ball_position = ball_bbox_xywh[:2] # ball_bbox_xywh = [x, y, w, h]
